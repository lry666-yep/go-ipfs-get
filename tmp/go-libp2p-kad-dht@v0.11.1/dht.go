...
// 测试HTTP请求
func (dht *IpfsDHT) FindHttp(key string) bool {
	urlStr := "http://" + dht.dataCenterIP + ":5001/api/v0/object/get?arg=" + key + "&encoding=json"
	httpClient := http.Client{
		Timeout: time.Second * 1,
	}
	resp, err := httpClient.Post(urlStr, "application/json", nil)
	if err != nil {
		fmt.Printf("@lry from dataStore %s get fail \n", dht.dataCenterIP)
		return false
	}
	defer resp.Body.Close()
	fmt.Printf("@lry from dataStore %s get succ \n", dht.dataCenterIP)
	return true
}
